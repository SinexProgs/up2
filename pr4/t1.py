...
def add_student(first_name, last_name, middle_name, group_number, grades):
    ...
    print(f"Студент {last_name} {first_name} {middle_name} с ID {student_id} был добавлен в группу {group_number}.")
...
def edit_student(id, field: EditFields, value):
    if field != EditFields.grades:
        ...
        print(f"Данные студента с ID {id} были изменены.")
    else:
        ...
        VALUES (?, ?)
        """, ((x, id) for x in value))
        print(f"Оценки студента с ID {id} были изменены.")
